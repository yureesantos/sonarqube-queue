    ...
    create(context) {
        ...
        function checkAccessor(accessor) {
            ...
            if (matchingFields.length > 0 &&
                ...
                const primaryMessage = `Refactor this ${accessorInfo.type} so that it actually refers to the property '${fieldToRefer.name}'.`;
                const secondaryLocations = [fieldToRefer.node];
                const secondaryMessages = ['Property which should be referred.'];
                context.report({
                    message: utils_1.toEncodedMessage(primaryMessage, secondaryLocations, secondaryMessages),
