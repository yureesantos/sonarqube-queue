...
exports.rule = {
    ...
    create(context) {
        ...
        return {
            ...
            'FunctionDeclaration, FunctionExpression, ArrowFunctionExpression': (node) => functionsWithParent.set(node, nodes_1.getParent(context)),
...
function toPrimaryLocation(node, context) {
    const func = node;
    return {
...
class FunctionComplexityVisitor {
    ...
    visit() {
        const visitNode = (node) => {
            let token;
            if (utils_1.isFunctionNode(node)) {
                if (node !== this.root) {
                    return;
                }
                else {
                    token = { loc: locations_1.getMainFunctionTokenLocation(node, this.parent, this.context) };
                }
            }
            else {
                switch (node.type) {
            ...
            visitor_1.childrenOf(node, this.context.getSourceCode().visitorKeys).forEach(visitNode);
        };
        visitNode(this.root);
    }
    getComplexityTokens() {
