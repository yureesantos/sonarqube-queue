...
function checkExpression(callExpression, thisTypeUsageMap, otherTypeUsageMap, hasIssue, tail, context) {
    ...
    if (variable && variable.defs.length !== 0) {
        ...
        if (otherTypeUsage && otherTypeUsage.loc && !hasIssue.includes(variable)) {
            const message = `Correct the use of this function; on line ${otherTypeUsage.loc.start.line} it was called with${tail} "new".`;
            context.report({
                node: callExpression.callee,
                message: utils_1.toEncodedMessage(message, [otherTypeUsage.callee]),
            });
...
function getVariable(node, context) {
    if (node.callee.type === 'Identifier') {
        return utils_1.getVariableFromName(context, node.callee.name);
    }
    return undefined;
}
